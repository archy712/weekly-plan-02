...
- **검색·필터·정렬·페이지네이션** — 제목/내용 키워드 검색, 부서 필터(기본값은 일반 사용자는 자기 부서·관리자는 전체), 기간 범위 필터(v1, "이번 주/이번 달/최근 3개월" 프리셋), 모든 컬럼 헤더 클릭으로 정렬, 20건 단위 페이지네이션
...
- **관리자 콘솔(v1)** — 대시보드·조직 관리·부서 관리·업무타입 관리·사용자 관리 5개 탭으로 구성되며 공통 헤더의 "관리자 콘솔" 링크로만 진입(관리자 전용). **모든 탭은 관리자 소속 조직 범위로 제한**된다(별도의 "전체 관리자" 등급 없이, 소속 부서로 자동 결정). 대시보드는 부서별·기간별·상태별·업무 타입별·중요도별 업무 현황을 요약 카드 4종·차트 6종(막대/조각별 비율·수치 라벨, 중요도 레이더 포함)으로 시각화하고, 조직 관리는 자기 조직 1건의 이름 수정·비활성화만 가능한 단일 카드, 부서 관리는 부서 추가·이름 변경·비활성화(소프트 삭제, 부서원·업무일지가 있으면 하드 삭제 자동 차단), 업무타입 관리는 조직별로 독립된 업무 타입을 동일한 CRUD 패턴으로 관리, 사용자 관리는 조직 범위 내 사용자를 검색·필터·정렬로 조회해 역할(`user`/`admin`)과 소속 부서를 변경(자기 강등 방지, 재로그인 없이 즉시 권한 반영)
...
기능별 상세 명세는 [`docs/PRD.md`](docs/PRD.md)를 참고하세요. 다음 단계로 계획 중인 고도화(v1) 기능 — 실시간 알림 — 은 아직 구현 전이며 [`docs/ROADMAP_v1.md`](docs/ROADMAP_v1.md)에서 계획을 확인할 수 있습니다.
