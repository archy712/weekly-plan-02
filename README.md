...
- **업무 타입 다중 분류(v1)** — 네트워크·클라우드·보안 등 10종 중 1개 이상을 체크박스로 다중 선택. 상세 페이지에서는 진행상태와 동일하게 별도 수정 모드 없이 체크/해제만으로 즉시 반영
...
- **PDF 리포팅** — 현재 조회 중인 목록(적용된 필터 포함)을 표 형태 PDF로 다운로드(한글 폰트 임베딩)
...
- **관리자 콘솔(v1)** — 대시보드·부서 관리·사용자 관리 3개 탭으로 구성되며 공통 헤더의 "관리자 콘솔" 링크로만 진입(관리자 전용). 대시보드는 부서별·기간별·상태별·업무 타입별 업무 현황을 요약 카드 4종·차트 5종(막대/조각별 비율·수치 라벨 표시)으로 시각화하고, 부서 관리는 부서 추가·이름 변경·비활성화(소프트 삭제, 부서원·업무일지가 있으면 하드 삭제 자동 차단), 사용자 관리는 전체 사용자를 검색·필터·정렬로 조회해 역할(`user`/`admin`)과 소속 부서를 변경(자기 강등 방지, 재로그인 없이 즉시 권한 반영)
...
- **shadcn `chart` + Recharts**(v1) — 관리자 콘솔 통계 대시보드 차트
