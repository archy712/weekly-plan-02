...
- **검색·필터·페이지네이션** — 제목/내용 키워드 검색, 부서 필터(기본값은 일반 사용자는 자기 부서·관리자는 전체), 기간 범위 필터(v1, "이번 주/이번 달/최근 3개월" 프리셋), 20건 단위 페이지네이션
...
- **관리자 콘솔(v1)** — 대시보드·부서 관리·사용자 관리 3개 탭으로 구성되며 공통 헤더의 "관리자 콘솔" 링크로만 진입(관리자 전용). 대시보드는 부서별·기간별·상태별 업무 현황을 요약 카드 4종·차트 4종으로 시각화하고, 부서 관리는 부서 추가·이름 변경·비활성화(소프트 삭제, 부서원·업무일지가 있으면 하드 삭제 자동 차단), 사용자 관리는 전체 사용자를 검색·필터로 조회해 역할(`user`/`admin`)과 소속 부서를 변경(자기 강등 방지, 재로그인 없이 즉시 권한 반영)
