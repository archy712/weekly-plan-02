...
- 테이블 컬럼 정렬 헤더는 `components/sortable-table-head.tsx`(`SortableTableHead`)로 공통화되어 있습니다 — 클릭 시 오름차순/내림차순을 토글하는 UI만 담당하는 순수 컴포넌트이고, 정렬 키·방향 state는 호출하는 쪽(`components/weekly-log-list-view.tsx`, `components/user-admin-table.tsx`)이 관리합니다. **실제 정렬은 클라이언트가 아니라 URL 쿼리파라미터(`sort`/`dir`) → 서버 `ORDER BY`로 처리합니다** — 두 목록 모두 무한 스크롤이라 이미 불러온 배치만 클라이언트에서 정렬하면 이어 붙는 다음 배치와 순서가 어긋나기 때문입니다. 새 테이블에 정렬을 추가할 때도 이 컴포넌트를 재사용하되, 정렬 키는 반드시 서버에서 `ORDER BY`로 표현할 수 있는 것만 두세요.
- 진행업무 목록의 **"작성자" 정렬은 PostgREST 계산 필드**(computed field)를 씁니다 — 작성자 이름은 `weekly_logs` 컬럼이 아니고 `profiles_select_own_or_admin` RLS 때문에 embed로도 가져올 수 없어(그래서 표시는 `get_profile_identities` RPC 경유) 원래 정렬 대상에서 빠져 있었는데, 테이블 복합 타입을 인자로 받는 `SECURITY DEFINER` 함수 `public.author_sort_name(weekly_logs)`(= `coalesce(nullif(btrim(name),''), email)`, 마이그레이션 `add_weekly_logs_author_sort_name_computed_field`)를 만들어 `order=author_sort_name`으로 서버 정렬이 가능해졌습니다. 이 프로젝트의 첫 계산 필드 도입이며 주의점은 세 가지입니다: (1) **값은 화면 표시 폴백(이름 → 이메일)과 항상 같아야** 정렬 결과가 화면 순서와 일치합니다, (2) `anon`은 EXECUTE를 회수하되 **`authenticated`에는 반드시 남겨야** 합니다(PostgREST가 `order=`를 처리할 때 호출 역할 권한으로 실행 — 회수하면 목록 전체가 401), (3) 계산 필드는 `select=*`에 포함되지 않아 기존 조회 payload는 그대로이지만, 정렬에 쓰면 행마다 `profiles` PK 조회가 일어나고 인덱스를 탈 수 없으므로 데이터가 수만 건대로 커지면 비정규화를 재검토해야 합니다. 검색어와 작성자 정렬이 동시에 걸리면 `.or()` 대신 두 `ilike` 결과를 JS로 병합하는 관례상 서버 정렬을 JS로 재현해야 하는데, 이때만 `get_profile_identities`를 한 번 더 호출해 정렬용 이름 맵을 만듭니다(`lib/queries/weekly-logs.ts`의 `fetchAuthorSortNames`).
...
- **노출 순서는 `work_types.sort_order`(integer NOT NULL, 기본값 0)가 결정합니다(ad hoc)** — 관리자 콘솔의 업무 타입 관리에서 행을 드래그해 정한 순서가 진행업무 등록/상세/수정 화면의 체크박스 순서에 그대로 반영됩니다(조회는 전부 `.order("sort_order").order("name")`). 순서는 **부문(organization) 안에서만** 의미가 있어 각 부문이 1..N을 따로 갖습니다.
  - 저장은 `reorder_work_types(work_type_ids uuid[])` RPC 1회 호출로, 배열 순서를 그대로 `sort_order` 1..N으로 기록합니다. `stats_*`와 동일하게 **`SECURITY INVOKER`**라 `work_types`의 UPDATE RLS가 그대로 판정하며(권한 없는 행은 예외 없이 조용히 제외), 호출부(`reorderWorkTypesAction`)가 **RPC가 돌려준 갱신 건수와 요청 개수를 비교**해 부분 실패를 잡습니다 — 이 판정이 성립하려면 "값이 이미 같은 행은 건너뛰기" 같은 조건을 RPC에 넣으면 안 됩니다(처음 그렇게 작성했다가 row_count가 요청 수와 어긋나 곧바로 고친 이력 있음). 서로 다른 부문의 id가 섞여 들어오면 RPC가 예외를 던집니다.
  - 신규 추가 시 순번은 `set_work_type_sort_order()` `BEFORE INSERT` 트리거가 그 부문의 맨 뒤(max+1)로 채웁니다. 컬럼 기본값 `0`은 "아직 지정 안 됨" 센티넬입니다 — NOT NULL만 두면 Supabase 타입 생성기가 `Insert`에서 이 컬럼을 필수로 표시해 모든 insert 호출부가 순번을 직접 계산해야 하기 때문입니다(실제 순서 값은 항상 1부터라 0과 겹치지 않음). 이 트리거와 `reorder_work_types`는 둘 다 `SECURITY INVOKER`라 `record_weekly_log_change_history()` 같은 `SECURITY DEFINER` 함수의 `anon`/`authenticated` revoke 관리가 필요 없습니다.
  - UI(`components/work-type-sortable-list.tsx`)는 **`@dnd-kit/sortable`을 새로 설치하지 않고 칸반보드와 동일하게 `@dnd-kit/core`만으로** 구현했습니다 — 행 하나가 `useDraggable`+`useDroppable`을 동시에 갖고, 놓인 행의 자리로 `arrayMove`하는 방식입니다(항목이 실시간으로 밀려나는 대신 드롭 대상 행을 테두리로 강조). 드래그 시작은 `setActivatorNodeRef`를 붙인 손잡이 버튼에서만 가능합니다(행 전체가 소스가 되면 수정/삭제 버튼 클릭과 충돌). **모바일 카드와 데스크탑 테이블이 같은 항목을 동시에 렌더링하므로 `DndContext`를 2개로 분리**했습니다 — 한 컨텍스트 안에서 같은 id가 중복 등록되면 dnd-kit이 깨지지만(칸반의 주의사항과 동일), 별도 컨텍스트는 레지스트리가 독립이라 id가 겹쳐도 무방합니다. `DragOverlay`는 자기 컨텍스트의 드래그만 그리므로 양쪽에 하나씩 둡니다. 키보드 정렬(KeyboardSensor)은 칸반과 마찬가지로 지원하지 않습니다.
  - **대시보드 업무타입 차트도 이 순서를 따릅니다** — `stats_logs_by_work_type` RPC의 `order by`를 `wt.sort_order, wt.name`으로 바꾸고(파라미터 목록은 그대로라 오버로드 함정 없음, `regprocedure`로 시그니처 1개만 남은 것을 실측 확인), `components/dashboard-worktype-chart.tsx`가 클라이언트에서 다시 하던 **건수 내림차순 재정렬을 제거**했습니다. RPC만 고치면 화면은 그대로이니 둘을 항상 같이 볼 것 — 이 재정렬을 되살리면 관리자가 정한 순서가 대시보드에 반영되지 않습니다.
...
- **`/tech-stack`**은 더 이상 `package.json`을 그대로 나열하며 npm 최신/설치 버전을 조회하지 않습니다 — `lib/queries/npm-versions.ts`(registry fetch + `"use cache"`/`cacheLife`)는 삭제되었습니다. 대신 `lib/constants/tech-stack.ts`에 실제로 눈에 띄는 기술만 고른 **큐레이션 정적 목록**(버전 배지·npm 링크·검색 없음)이 있습니다. 한때 아직 도입 전인 배포/CI 도구를 담은 `PLANNED_TECH_STACK`("예정" 배지, 점선 카드)이 따로 있었지만 지금은 없습니다 — GitHub Actions는 도입 계획이 없어 항목째 삭제했고(저장소에 `.github/workflows`도 없음), Vercel은 실제 프로덕션 운영 중이라 본 목록의 "배포 & 운영" 카테고리로 옮겼습니다. 다시 "예정" 항목을 두고 싶다면 상수와 뷰의 전용 섹션을 함께 되살려야 합니다(뷰는 `TECH_STACK` 카테고리만 순회하도록 단순해진 상태).
