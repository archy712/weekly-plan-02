...
- **Task 039: 애플리케이션 전반 성능 개선 (F032)**
  - [ ] **Task 037과의 경계 확정 (착수 전 필수)** — Task 037은 **v1 신규 테이블 4종·신규 화면에 한정된 배포 전 점검**이고, 이 Task는 **MVP 포함 애플리케이션 전체를 대상으로 하는 별도의 성능 개선 이니셔티브**다. 두 Task가 같은 항목을 두 번 손대지 않도록 착수 시점에 Task 037의 실제 수행 결과를 확인하고 중복분을 제외할 것
  - [ ] **측정 먼저, 최적화는 그 다음** — 추측으로 코드를 고치지 않는다. 개선 전/후를 같은 방법으로 재현할 수 있도록 측정 절차와 기준값을 **먼저** 기록
  - [ ] **DB 쿼리·인덱스 점검**
    - [ ] `mcp__supabase__get_advisors`(performance)의 **기존 미해결 경고 정리** — 예: `weekly_log_attachments`의 미인덱스 FK 2건. "의도된 설계로 확인된 기존 경고"와 실제 개선 대상을 구분해 기록(MVP·Task 037에서 쓰던 분류 방식 유지)
    - [ ] N+1·중복 조회 후보 실측 — 목록 페이지의 댓글수 2차 조회, 관리자 콘솔 부서/업무타입 목록의 참조 건수 카운트, 사용자 관리 목록의 부서 조인, 대시보드 `stats_*` RPC 6종을 `EXPLAIN ANALYZE`로 확인
    - [ ] **데이터 증가 대비 확장성** — 현재 규모(`weekly_logs` 167건 기준으로 설계된 부분)가 10배·100배가 됐을 때 무너지는 지점 식별. 특히 **클라이언트 사이드 정렬**(`components/sortable-table-head.tsx` 계열, 현재 페이지 20건 한정 전제)과 **PDF·Excel 다운로드가 필터 결과 전체를 한 번에 불러오는 구조**
  - [ ] **렌더링·캐싱 점검** — `cacheComponents: true` 하에서 Suspense 경계가 실제로 스트리밍 이득을 내고 있는지, 보호 페이지마다 반복되는 부서 게이트 조회(`profiles.department_id`)가 요청당 몇 번 발생하는지, 사용자별 데이터에 캐시가 잘못 걸린 곳은 없는지 재검토
  - [ ] **클라이언트 번들 점검** — `npm run build`로 라우트별 First Load JS를 기록하고, 동적 로딩(`await import`)이 걸려 있는 jsPDF·exceljs·recharts가 **실제로 초기 번들에 포함되지 않는지 프로덕션 네트워크로 실측**(Task 037의 recharts 점검을 전 라이브러리로 확장). Tiptap이 작성·상세 라우트 밖으로 새지 않는지도 확인
  - [ ] **정적 자산 점검** — PDF용 한글 폰트(`/public/fonts/NotoSansKR-Regular.ttf`, ~2.5MB)가 PDF 생성 시점에만 fetch되는지와 캐시 헤더 적절성 확인
  - [ ] **효과가 측정된 변경만 반영** — 개선 폭이 없거나 회귀 위험이 큰 변경은 적용하지 않고 **기각 근거를 남긴다**(나중에 같은 시도를 반복하지 않기 위함, 이 프로젝트의 실측 기록 관례)
  - [ ] 결과 요약을 `docs/guides/deployment-ops.md`(운영 점검 절차)와 이 로드맵에 기록
  - **관련 파일**: 측정 결과에 따라 결정하므로 사전에 고정하지 않는다. 우선 조사 대상은 `app/protected/weekly-logs/page.tsx`, `app/protected/admin/**`, `lib/queries/stats.ts`, `components/weekly-log-list-view.tsx`, `lib/pdf/weekly-log-pdf.ts`, `lib/excel/weekly-log-excel.ts`, `proxy.ts`
  - **수락 기준**: 목표 지표를 세우기로 확정한 경우 합의된 지표를 만족하고, 일반 점검으로 확정한 경우 **측정 → 개선 → 재측정 기록**이 남고 어드바이저 performance 경고가 "해소" 또는 "의도된 설계"로 전부 분류된다. 어느 쪽이든 **기존 기능에 회귀가 없어야 한다**
  - **테스트 체크리스트**
    - [ ] 개선 전/후 비교표 작성(라우트별 First Load JS, 주요 쿼리 `EXPLAIN ANALYZE` 실행시간, 주요 페이지 체감 로딩 시간)
    - [ ] Playwright MCP로 주요 사용자 플로우 전수 재실행 — 성능 변경이 기능 회귀를 만들지 않았는지 확인(목록 4종 필터·작성·상세·댓글·관리자 콘솔 5개 탭·대시보드)
    - [ ] 캐싱을 건드린 경우 **사용자별·조직별 데이터가 다른 사용자에게 새지 않는지** 계정 2개(다른 부서·다른 역할)로 교차 확인 — **이 Task에서 가장 위험한 회귀**
    - [ ] 인덱스를 추가한 경우 쓰기 경로(작성·수정·댓글)가 느려지지 않았는지 함께 측정
    - [ ] `npm run build` + `next start` 프로덕션 빌드에서 재측정하고 콘솔 에러·하이드레이션 경고 0건 확인
  - **범위 밖 유지**: 인프라 변경(Supabase 플랜 상향, CDN·엣지 캐시 신규 도입), 아키텍처 재작성 수준의 리팩터링, 부하 테스트 도구·APM/모니터링 서비스 신규 연동, 성능을 이유로 한 기존 기능 축소
