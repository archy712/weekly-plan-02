...
- **Task 043: 알림 구독 설정 UI 및 트리거 게이트 적용 (F044)**
  ...
  - [ ] **`notify_on_comment_mention()` 트리거 수정** — 멘션 대상의 `notify_on_mention`이 `false`면 알림을 만들지 않는다
  - [ ] **⚠️ 게이트는 "알림 행 생성"만 막고 댓글·멘션 자체는 절대 막지 않을 것** — 두 함수는 `AFTER INSERT` 트리거이므로 `return new`를 유지해 **댓글/멘션 행 저장은 항상 성공**해야 한다. 여기서 예외를 던지면 알림 설정이 댓글 기능을 망가뜨린다
  - [ ] **설정 저장 방식 결정** — 프로필 폼(`components/profile-form.tsx`, 이미 RHF+Zod)에 필드로 편입 ← 권장(신규 화면·신규 라우트 불필요). 단 **저장 버튼을 눌러야 반영되는 폼 방식** vs **토글 즉시 저장(낙관적 업데이트)** 중 하나를 고른다 — 나머지 프로필 필드와의 일관성을 위해 **폼 방식 권장**, 즉시 저장을 택한다면 `weekly-log-detail-view.tsx`의 낙관적 업데이트 패턴을 그대로 재사용
  - [ ] `npx shadcn@latest add switch` — `components/ui/switch.tsx`가 **현재 미설치**(실측 확인). 설치 후 라이트/다크 양쪽 대비 확인
  - [ ] `lib/schemas/profile.ts`에 boolean 3종 추가, `app/protected/profile/page.tsx`의 select에 컬럼 3종 추가
  - [ ] `components/notification-preferences-field.tsx` 신규(또는 프로필 폼 내 섹션) — 3개 스위치 + 각 항목 설명 캡션. **"대댓글 알림은 댓글 설정을 따릅니다"를 캡션에 명시**(Task 042의 매핑을 사용자에게 드러냄)
  - [ ] **리마인더 스위치는 이 Task에서 함께 노출하되 "아직 동작하지 않음"이 아니라 그냥 저장만 되게 둔다** — Task 044가 이 값을 읽기 시작하면 추가 UI 작업 없이 활성화된다
  ...
  - **수락 기준**: 사용자가 프로필 화면에서 댓글·멘션·리마인더 알림을 각각 끌 수 있고, 끈 유형의 알림은 **DB에 행 자체가 생성되지 않으며**(읽음 처리로 숨기는 방식이 아님), 알림을 꺼도 댓글 작성·멘션 자체는 정상 동작한다
  - **테스트 체크리스트** (Playwright MCP + Supabase MCP. 임시 QA 계정 2개(작성자·수신자)를 실제 가입 플로우로 생성해 실브라우저 검증, 종료 후 완전 삭제)
    - [ ] 수신자가 댓글 알림을 끈 상태에서 타인이 그의 업무일지에 댓글 → **`notifications` 행이 생성되지 않고** 헤더 벨 배지도 증가하지 않음 확인
    - [ ] 같은 상황에서 **댓글 자체는 정상 저장되고 상세 페이지에 즉시 표시**되는지 확인(게이트가 기능을 막지 않는다는 증거)
    - [ ] 멘션 알림만 끄고 댓글 알림은 켠 상태에서 **댓글 + 멘션이 동시에 발생**하는 케이스 → 댓글 알림만 생성되는지 확인(두 트리거가 독립적으로 게이트되는지)
    - [ ] 대댓글 알림이 `notify_on_comment`(댓글 설정)를 따르는지 확인 — 댓글 알림을 끄면 대댓글 알림도 생성되지 않음
    - [ ] 설정을 다시 켠 뒤 새 댓글 → 알림이 정상 생성되는지 확인(되돌릴 수 있음)
    - [ ] 이미 생성된 과거 알림은 설정을 꺼도 **사라지지 않는지** 확인(설정은 미래 알림에만 적용)
    - [ ] Realtime 회귀: 알림이 켜진 수신자 화면에서 **새로고침 없이** 벨 배지가 증가하는지 확인(v1 Task 035 동작 회귀 없음)
    - [ ] 설정 저장 실패(네트워크 가로채기) 시 토스트가 뜨고 값이 롤백되는지 확인
    - [ ] 라이트/다크 + 1280/768/390 뷰포트에서 스위치 레이아웃·대비 확인
    - [ ] 콘솔 에러 0건
