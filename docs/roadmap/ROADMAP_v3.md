...
- **Task 069: 문서 2차 동기화 — F063~F065 반영 및 PRD 화면 표기 리네임 (F062 후속)** ✅
  ...
  - **범위 밖 유지**: `docs/roadmap/ROADMAP_mvp.md`·`ROADMAP_v1.md`·`ROADMAP_V2.md`와 `docs/guides/` — **과거 시점의 개발 이력 기록**이라 당시 표기를 그대로 두는 편이 이력으로서 정확하다(PRD는 "현재 제품의 명세"라 최신 표기를 따라야 한다는 점에서 성격이 다름)
