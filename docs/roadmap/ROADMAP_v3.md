...
- **Task 069: 문서 2차 동기화 — F063~F065 반영 및 PRD 화면 표기 리네임 (F062 후속)** ✅
  ...
  - [x] **`docs/roadmap/` 리네임(사용자 추가 요청)** — 처음에는 로드맵을 "과거 시점의 개발 이력이라 당시 표기를 두는 편이 정확하다"며 범위 밖으로 뒀으나, 사용자가 리네임을 지시해 `ROADMAP_mvp.md`(23곳)·`ROADMAP_v1.md`(23곳)·`ROADMAP_V2.md`(8곳)을 치환했다(잔존 0건). `ROADMAP_codereview.md`는 해당 표기가 없어 대상 아님
  - [x] **`ROADMAP_v3.md`는 의도적으로 손대지 않음** — 이 문서의 "주간업무"/"업무일지" 11곳은 전부 **리네임 작업 자체를 서술하는 문장**이다(Task 064의 치환 규칙과 조사 건수, Task 065의 CLAUDE.md 서술 10곳, Task 069의 PRD 치환 건수, F061 커버리지 매핑 등). 치환하면 "진행업무를 진행업무로 리네임했다"가 되어 기록이 무의미해진다
  - [x] 로드맵에서도 일괄 치환 전 **개별 처리** — `ROADMAP_V2.md`의 F041 도입 이유(PRD와 같은 말장난 문장) → "주 단위로 기록하는 업무인데...", `ROADMAP_mvp.md`의 "주간 업무"(띄어쓰기 변형) 3곳 → "업무"(그대로 치환하면 "진행업무 업무"가 됨). 붙여쓰기 표현 "주간업무목록" 3곳은 "진행업무 목록"으로 띄어쓰기까지 교정
  - **범위 밖 유지**: 없음(프로젝트 문서 전체가 정리됨). 단 **코드 주석·식별자·라우트·DB 스키마의 `weekly_log*` 표현은 계속 유지**된다 — F061이 처음부터 화면 표기만 대상으로 했고, CLAUDE.md도 이 원칙을 명시하고 있다
