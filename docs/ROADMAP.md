...
- **Task 005: 랜딩 페이지 UI 구현 (F015)**
  ...
  - [ ] 주요 기능 소개 카드 4종: 주간업무일지 작성/조회, 부서별 관리, 완료 상태 추적, PDF 다운로드
  - [ ] [로그인] / [회원가입] CTA 버튼 배치 및 라우팅 연결
  - [ ] 데스크탑/태블릿/모바일 3단 반응형 그리드, 라이트·다크 양쪽 시각 검증
  - **수락 기준**: 비로그인 방문자가 랜딩에서 로그인·회원가입 페이지로 이동 가능
