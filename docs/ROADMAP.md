...
- **Task 024: 아바타 헤더 노출, 프리셋 확장·팝업 선택 UX, 인증 화면 반응형·회원가입 필드 확장 (F018 보강)** ✅ (2026-08-04)
  ...
  - [x] **후속 수정 (2026-08-04, 사용자 재확인)**: "프로필 화면처럼 전체를 차지"한다는 요청이 512px 상한으로는 충분히 반영되지 않았다는 피드백에 따라, `app/protected/layout.tsx`의 콘텐츠 영역과 동일한 `max-w-5xl`으로 2차 확장. Playwright로 1280px(데스크탑, 카드가 프로필 화면과 동일한 폭까지 확장됨을 스크린샷 대조)·390px(모바일, `w-full`에 의해 기존과 동일하게 화면 폭에 맞춰 축소됨을 확인) 재검증
  - [x] **최종 원복 (2026-08-04, 사용자 재확인)**: 로그인/회원가입 화면은 프로필 화면과 달리 필드 수가 적은 짧은 폼이라 넓은 폭이 오히려 어색하다는 판단에 따라, 사용자가 두 차례의 확장 시도를 모두 되돌리고 원래의 고정 `max-w-sm`으로 복귀 요청. `app/auth/login/page.tsx`·`app/auth/sign-up/page.tsx`를 최초 스타터킷 상태(`max-w-sm`)로 되돌림 — 이 두 페이지의 폭은 더 이상 반응형으로 확장하지 않는 것으로 최종 확정
