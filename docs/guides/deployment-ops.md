...
- [ ] **(v2)** 목록 페이지 상단 "내 업무" 요약 위젯이 오류 없이 렌더링(F040)
- [ ] **(v2)** 작성 페이지에서 새로고침 시 임시저장 배너가 정상 동작(F042)
- [ ] **(v2)** 상세 페이지에서 진행상태를 바꾸면 변경 이력 섹션에 1건이 기록(F043)
- [ ] **(v2)** 프로필 페이지의 알림 구독 설정 스위치 3종이 저장(F044)
- [ ] **(v2)** 알림 벨이 정상 동작(리마인더는 매주 금요일 발송이라 스모크 시점에 즉시 재현되지 않을 수 있음 — `cron.job_run_details`로 대체 확인)
- [ ] **(v3)** 관리자 계정으로 상세 페이지에서 담당자 이관 → 담당자·소속 팀이 함께 바뀌고, 이관 이력 1건과 "이관 N회" 배지가 생기며, 새 담당자에게 알림이 도착(F063). 일반 사용자 계정에서는 [이관] 버튼 자체가 노출되지 않아야 함
- [ ] **(v3)** 상세 페이지에서 진척률 슬라이더를 바꾸면 변경 이력에 1건이 기록(F064)
...
상세 페이지에서 진행상태·업무타입·업무중요도를 인라인으로 바꿀 때마다 `weekly_log_change_history`에 행이 1건씩 쌓입니다(`AFTER UPDATE` 트리거 전용 기록, CLAUDE.md "변경 이력" 절 참고). v2 착수 시점(2026-08-21) 기준 이 테이블은 **v2가 추가한 테이블 중 인라인 편집마다 계속 늘어나는 유일한 테이블**이라 다른 신규 테이블(1회성 스키마 확장)과 증가 특성이 다릅니다.

**현재 정책**: 별도 삭제 정책을 두지 않고 무기한 보존합니다. 근거:

- 최소 버전 이력이라 표시 자체가 이미 조회 시 최근 50건 상한(`HISTORY_PAGE_SIZE`)으로 제한되어 있어, 오래된 행이 쌓여도 상세 페이지 렌더링에는 영향이 없습니다(조회는 `weekly_log_id` 인덱스로 좁혀지고, 표시 상한이 조회 자체를 짧게 유지).
- `notifications`(90일 후 자동 소멸이 자연스러운 "읽었으면 끝난 알림")과 달리, 변경 이력은 "누가 언제 무엇을 바꿨는지"의 감사 근거라 시간 경과만으로 삭제하는 것이 정책적으로 맞지 않습니다.
- v2 착수 시점 실측 데이터 규모(325건 로그)에서 이 테이블은 아직 인덱스 없이도 가벼운 수준입니다.

**규모가 커지면 검토할 것**: `weekly_logs` 삭제 시 `on delete cascade`로 이력도 함께 삭제되므로 로그가 유지되는 한 이력도 함께 늘어납니다. 실사용에서 특정 로그의 변경이 50건(표시 상한)을 초과하는 사례가 관측되거나, 테이블 전체 행 수가 `weekly_logs`의 몇 배를 넘어 저장 공간이 신경 쓰이는 시점이 오면, 7절과 동일한 패턴("N일/N개월 경과 후 삭제" 또는 "로그당 최근 N건만 유지")으로 수동 SQL 또는 `pg_cron` 잡을 추가하세요(잡 이름 접두사 `weekly_log_` 유지). 이력 기반 되돌리기(revert) 기능이 없으므로 삭제해도 기능이 깨지지 않는다는 점이 정리 결정을 단순하게 만듭니다.

```sql
-- 규모 확인용(정기 점검 시 참고)
select count(*) from weekly_log_change_history;
select weekly_log_id, count(*) from weekly_log_change_history group by weekly_log_id order by count(*) desc limit 10;
```
